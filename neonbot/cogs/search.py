...
class Search(commands.Cog):
  ...
  async def image(self, ctx, *, args):
    ...
    res = await self.session.get("https://www.googleapis.com/customsearch/v1",
                                 params={
                                   "q": args,
                                   "num": 1,
                                   "searchType": "image",
  ...
  async def dictionary(self, ctx, *, args):
    msg = await ctx.send(embed=Embed(description="Searching..."))
    res = await self.session.get(f"https://www.dictionaryapi.com/api/v3/references/sd4/json/{args}",
  ...
  async def weather(self, ctx, *, loc):
    msg = await ctx.send(embed=Embed(description="Searching..."))
    res = await self.session.get(f"http://api.openweathermap.org/data/2.5/weather",
                                 params={
                                   "q": loc,
                                   "units": "metric",
    ...
    await msg.delete()
    if json.cod != 200:
      return await ctx.send(embed=Embed(description="City not found."), delete_after=5)

    embed = Embed(title=f":flag_{json.sys.country.lower()}: {json.sys.country} - {json.name}",
    ...
    embed.set_thumbnail(url=f"http://openweathermap.org/img/w/{json.weather[0].icon}.png")
    ...
    embed.add_field(name="🌤 Sunrise",
                    value=datetime.fromtimestamp(json.sys.sunrise).strftime("%b %d, %Y %-I:%M:%S %p"),
                    inline=False)
    embed.add_field(name="🌥 Sunset",
                    value=datetime.fromtimestamp(json.sys.sunset).strftime("%b %d, %Y %-I:%M:%S %p"),
                    inline=False)
    embed.add_field(name="🔘 Coordinates",
                    value=f"Longitude: {json.coord.lon}\nLatitude: {json.coord.lat}",
                    inline=False)
    embed.add_field(name="🎛 Pressure", value=f"{json.main.pressure} hpa", inline=False)
  ...
  async def lol(self, ctx, *, champion):
    ...
    res = await self.session.get(f"https://www.leaguespy.net/league-of-legends/champion/{champion}/stats")
    await loading_msg.delete()
    if res.status == 404: return await ctx.send("Champion not found.")
    html = await res.text()
    ...
    strong_against = [
      champ_counter[0].select("div.champ__counters__radials__big > a > span")[0].get_text(),
      champ_counter[0].select("div.champ__counters__radials__small > a > span")[0].get_text(),
      *[row.find("a").get_text().strip() for row in champ_counter[0].find_all("div", "ls-table__row")]
    ]

    weak_against = [
      champ_counter[1].select("div.champ__counters__radials__big > a > span")[0].get_text(),
      champ_counter[1].select("div.champ__counters__radials__small > a > span")[0].get_text(),
      *[row.find("a").get_text().strip() for row in champ_counter[1].find_all("div", "ls-table__row")]
    ]
    ...
    for row in item_block:
      ...
      for top in row.select(".item-block__top"):
        arr += [i.get_text() for i in top.select(".item-block__items > span > span")]
      item_build.append(arr)

    rune_block = soup.find("div", "champ__buildLeftNew").select("div.rune-block.rune-block--new")[0]
    ...
    info = Dict({
      "name":
        soup.select(".champ__header__left__main > h2")[0].get_text(),
      "icon":
        soup.select(".champ__header__left__radial img")[0].get("src"),
      "role":
        soup.select(".stat-source > .stat-source__btn[active=true] > a")[0].get_text().split(" ")[0],
      "role_icon":
        "https://www.leaguespy.net" +
        soup.select(".champ__header__left__radial > .overlay > img")[0].get("src"),
      "win_rate":
        soup.select(".champ__header__left__main > .stats-bar")[0].find("span").get_text(),
      "ban_rate":
        soup.select(".champ__header__left__main > .stats-bar")[1].find("span").get_text(),
    })
    ...
    embed.set_footer(text="Powered by LeagueSpy",
  ...
  async def lyrics(self, ctx, *, song):
    loading_msg = await ctx.send(embed=Embed(description="Searching..."))
    res = await self.session.get("https://search.azlyrics.com/search.php", params={"q": song})
    html = await res.text()
    soup = BeautifulSoup(html, 'html.parser')
    links = [
      Dict({
        "title": link.find("b").get_text(),
        "url": link.get("href")
    ...
    embed.set_footer(text="Powered by AZLyrics", icon_url="https://www.azlyrics.com/az_logo_tr.png")
